...
class Controls:
    ...
    Class Controls containing the methods to control:
        ...
        - site_select       : select a news site depending on what news site button is pressed
    ...
    def create(self):
        '''
            Method to create control buttons, including news sites defined in the Dictonary
        '''
        width = 10
        self.root.grid_rowconfigure(0, weight=1)
        self.root.grid_columnconfigure(0, weight=1)
        
        self.pause_button = Button(self.root, text='Pause', width=width, command = self.pause_status)
        self.pause_button.pack(anchor='e')

        self.next_button  = Button(self.root, text='Next', width=width, command = self.next_item)
        self.next_button.pack(anchor='e')  

        self.previous_button = Button(self.root, text='Prev.', width=width, command = self.previous_item)
        self.previous_button.pack(anchor='e')

        self.exit_button     = Button(self.root, text='Exit', width=width, command=self.exit_news)
...
class Display:
    ...
    def main_title(self, title_text):
        ...
        title_main = Label(title_frame, text=title_text, bd=-1, fg='black',bg='white', \
                           font=self.font_main_title)
        title_main.place(x = int(self.title_x/2), y = -self.padding+32, anchor = 'center')

    def news_window(self):
        '''
        Method to display the news window which will contain the news item title, 
        news summary
        '''
